...
public class MemoryTool {
    ...
    public long getModuleAddress(String moduleName) {
        try {
            // false = no extended info (no PDB/version loading) - cheaper and
            // less prone to transient native failures at high call rates
            return process.moduleGet(moduleName, false).getVaBase();
        } catch (Exception e) {
            // silent: the caller (GameDataManager) decides when/how to log
            return 0;
        }
    }
    ...
    /** Raw memory read; returns null on failure (no noisy logging). */
    public byte[] readBytes(long va, int size) {
        try {
            return process.memRead(va, size, 1);
        } catch (Exception e) {
            return null;
        }
    }
