...
public class PlayerAddressUpdateThread extends Thread {
    ...
    /**
     * Reload offsets.json at runtime (used after the auto-updater rewrites the
     * file, so a game restart picks up fresh offsets without restarting the client).
     */
    public static synchronized boolean reloadOffsets() {
        try {
            String json = new String(java.nio.file.Files.readAllBytes(
                    new java.io.File("offsets.json").toPath()), java.nio.charset.StandardCharsets.UTF_8);
