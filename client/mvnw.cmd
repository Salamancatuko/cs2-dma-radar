...
@REM JAVA_HOME is not set - try to derive it from java.exe on PATH
set "JAVA_CMD="
for /f "delims=" %%i in ('where java 2^>nul') do if not defined JAVA_CMD set "JAVA_CMD=%%i"
if not defined JAVA_CMD goto noJavaDerived
...
:noJavaDerived
echo.
echo Error: JAVA_HOME is not set and no JDK java.exe could be found. >&2
echo Please install a JDK 17+ (e.g. Temurin) or set JAVA_HOME to its location. >&2
echo.
goto error
